...
function calculate() {
	...
	// Purchase Price From Slider
	var per_val=parseInt(localStorage.pSlider);
	// Mortage Rate From Slider
	...
	// =(C2*(C6/12))/((1 - (1 + (C6/12))^(-360)))
	
	//alert("Mor Rat Fin: "+mog_rat_fin);
	
	var n_val = per_val * mog_rat_fin;
	
	//alert("N Val: "+n_val);
	
	var d_l_val = parseFloat(1) + parseFloat(mog_rat_fin);
	...
	var d_val = (1 - (1 / (Math.pow(d_l_val,d_r_val))));
...
function calculateMotgageGraph(check){
	...
	if(check == 1){
		...
		// Amout Financed
		var mog_pri = parseFloat(per_val) - parseFloat(dwn_pay_amt);
		//alert(localStorage.modpay);
		
		//loan balance
		var loan_amt=mog_pri*(1+(mog_rat/12))-localStorage.modpay;
		//alert("First"+loan_amt);
		localStorage.last_loan_stm = loan_amt;
		//equity
		var equity=per_val-loan_amt;
		//alert('loop true');
		return_val=equity;
	}
	else {
		...
		var loan_amt=localStorage.last_loan_stm*(1+(mog_rat/12))-localStorage.modpay;
...
//=================OTHER FUNCTION======================
function fixdollor(element){
	var numDoller=$(element).val().toString();
	var numDoller=numDoller.length;
	if(numDoller==4){ $('.dollor').css('right', '48px');}
	if(numDoller==5){ $('.dollor').css('right', '56px');}
	if(numDoller==6){ $('.dollor').css('right', '68px');}
	if(numDoller==7){ $('.dollor').css('right', '76px');}
}
$(document).ready(function(){
	
});
/*function putcommas(x){
		var pur_r=$(x).val().toString();
		var count=0;
		var num=new Array();
		while(pur_r>0){
			pur_r=pur_r/10;
